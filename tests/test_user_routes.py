...
def test_if_new_user_email_already_exists_in_system_and_return_422_if_open(http_test_client):
    pass
...
def test_given_is_active_is_true_for_an_existent_user_system_must_return_200_ok(http_test_client):
    with patch('class_social.users.user_controller') as controller_mock:
        controller_mock.get_user_by_is_active = Mock(return_value=users_list)
